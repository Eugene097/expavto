...
<body>
    ...
    <main>
        <div class="menuAdmin">
            ...
            <a href="searchUser.php" style="background-color: rgb(232, 232, 232);">Поиск пользователей</a>
        ...
        <form method="post">
            <div class="entryField input-group">
                <select name="options" class="btn custom-select" id="inputGroupSelect01" onchange="selectReady()">
                    <option selected disabled>Опции</option>
                ...
                <input type="text" class="form-control" disabled name="searchUser" id="searchUser" placeholder="Поиск пользователя" aria-label="Text input with segmented dropdown button">
                <div class="input-group-append">
                    <button type="submit" disabled id="search" name="search" class="btn btn-outline-secondary">Поиск</button>
                </div>
            </div>
        </form>
        <form method="post">
            ...
            if (isset($_POST["search"])) {
                if ($_POST["options"] == "phone") {
                    $phone = $_POST["searchUser"];
                    $query_users = "SELECT * FROM users WHERE phone LIKE '%$phone%'";
                }

                if ($_POST["options"] == "FIO") {
                    $fio = $_POST["searchUser"];
                    for ($i = 0; $i < strlen($fio); $i++) {
                        if ($fio[$i] == " ") $countSpace++;

                        if ($countSpace == 0 && $fio[$i] != " ") $surname = $surname . $fio[$i];
                        else if ($countSpace == 1 && $fio[$i] != " ") $name = $name . $fio[$i];
                        else if ($countSpace == 2 && $fio[$i] != " ") $patronymic = $patronymic . $fio[$i];
                    }
                    $query_users = "SELECT * FROM users WHERE surname LIKE '%$surname%' AND name LIKE '%$name%' AND patronymic LIKE '%$patronymic%'";
                }
        
                if ($_POST["options"] == "surname") {
                    $surname = $_POST["searchUser"];
                    $query_users = "SELECT * FROM users WHERE surname LIKE '%$surname%'";
                }

                if ($_POST["options"] == "name") {
                    $name = $_POST["searchUser"];
                    $query_users = "SELECT * FROM users WHERE name LIKE '%$name%'";
                }

                if ($_POST["options"] == "patronymic") {
                    $patronymic = $_POST["searchUser"];
                    $query_users = "SELECT * FROM users WHERE patronymic LIKE '%$patronymic%'";
                }
                $result_users = mysqli_query($link, $query_users);
                if (mysqli_num_rows($result_users) == 0) {
            ?> <h2 class="notFound">Пользователи не найдены</h2> <?
                } 
                else {
                    ?>
                    <table class="users">
    ...
    <script>
        function selectReady() {
            ...
            var textInput = document.getElementById("searchUser");
            ...
            if(select=="phone")
            {
                ...
                textInput.classList.add("jmp__input_tel");
                $(function() {
            ...
            if (select == "FIO") {
                textInput.placeholder = "Фамилия Имя Отчество";
            }
